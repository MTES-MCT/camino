...
class Instance {
  ...
  dispatch(type, detail, bubbles, cancelable) {
    const event = new CustomEvent(type, { detail, bubble: bubbles === true, cancelable: cancelable === true })
...
const ordereds = ['[tabindex]:not([tabindex="-1"]):not([tabindex="0"])']
...
const HeaderSelector = {
  ...
  BUTTONS: `${api.internals.ns.selector('header__tools-links')} ${api.internals.ns.selector('btns-group')}, ${api.internals.ns.selector('header__tools-links')} ${api.internals.ns.selector(
    'links-group'
  )}`,
