...
export const Utilisateur = defineComponent({
  ...
  onMounted(async () => {
    userUnwatch.value = watch(() => user, () => get())
  ...
  const isMe = computed(() => {
    return user.value && utilisateur.value.status ===  'LOADED' && user.value.id === utilisateur.value.value.id
  })
  ...
  const get = async () => {
    ...
    try {
      const utilisateurFromApi = await apiClient.getUtilisateur(utilisateurId)
      utilisateur.value = { 
        status: 'LOADED', 
    ...
    return () => (<div>
      ...
      <Accordion class="mb" slotSub={true} slotButtons={true}>
        {{ 
          ...
          {subscription.value.status === 'LOADED' ? (<button
          ...
          <div class="tablet-blobs">
            ...
            <div class="tablet-blob-3-4">
              <LoadingElement data={subscription.value} renderItem={(item) => (<p>{item ? 'Inscrit' : '–'}</p>) } />
            </div>
          </div>

          <LoadingElement data={utilisateur.value} renderItem={(item) => (<>{item.entreprises?.length ? (<div class="tablet-blobs">
            <div class="tablet-blob-1-4">
              <h5>
          ...
          <LoadingElement data={utilisateur.value} renderItem={(item) => (<>{isAdministration(item) ? (<div class="tablet-blobs">
            <div class="tablet-blob-1-4">
            ...
            <div class="tablet-blob-3-4">
              {
                `${Administrations[item.administrationId].abreviation}${
                  Administrations[item.administrationId].service ? ` - ${Administrations[item.administrationId].service}` : ''
                }`
              }
            </div>
          </div>) : null}</>) } />
              ...
              {isMe.value ? (<QGisToken generateTokenCall={() => fetchWithJson(CaminoRestRoutes.generateQgisToken, {}, 'post')} />) : null}
        </div>)
      }}
      </Accordion>
      {removePopup.value && utilisateur.value.status === 'LOADED' ? (<RemovePopup 
      close={() => removePopup.value = !removePopup.value}
      utilisateur={utilisateur.value.value}
      deleteUser={deleteUser}
    />) : null}
    {editPopup.value && utilisateur.value.status === 'LOADED' && subscription.value.status === 'LOADED' ? (<EditPopup 
