...
type FormatedLabel<FiltreId> = { id: FiltreId; name: string; value: string | string[]; valueName?: string | string[] }
...
export const Filters = defineComponent(
  <FiltreId extends string, SelectElement extends { id: SelectElementId }, SelectElementId extends string, AutocompleteElementId extends string>(
    props: Props<FiltreId, SelectElement, SelectElementId, AutocompleteElementId>
  ) => {
          ...
          default: () => (
            <div class="px-m">
              <div class="tablet-blobs mt">
                {inputs.value.length || autocompletes.value.length ? (
                  <div class="tablet-blob-1-2 large-blob-1-3">
                    ...
                    {autocompletes.value.map(input => (
                      <div key={input.id}>
                        <InputAutocomplete filter={input} />
                      </div>
                    ))}
                ...
                {selects.value.map(filter => (
                  <FiltersSelects key={filter.id} filter={filter} class="tablet-blob-1-2 large-blob-1-3" />
                ))}
