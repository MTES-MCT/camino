...
const saveLink = async () => {
  ...
  return () => (
    <div>
    <LoadingElement data={titresLinks.value} renderItem={(item) => (
      <>
      {item.amont.length || canLink.value ? (<div>
        <h5>Titre{ linkConfig.value && linkConfig.value.count === 'multiple' ? 's' : '' } à l’origine de ce titre</h5>
        {mode.value === 'edit' ? (<div>
          {titreLinkConfig.value ? (<TitresLink config={titreLinkConfig.value} loadLinkableTitres={props.apiClient.loadLinkableTitres(props.titre.typeId, props.titre.demarches)} onSelectTitre={() => {}} onSelectTitres={onSelectedTitres} />) : null}
          
          <div class="flex mt-m" style="flex-direction: row-reverse">
            <button class="btn-primary ml-s" style="flex: 0 1 min-content" onClick={saveLink}>Enregistrer</button>
            <button class="btn-secondary" style="flex: 0 1 min-content" onClick={() => mode.value = 'read'}>Annuler</button>
          </div>
        </div>) : (
          <div class="flex flex-center">
          <ul class="list-inline" style="margin-bottom: 0">
            {item.amont.map(titreFrom => (<li key={titreFrom.id} class="mr-xs">
          ...
          { canLink.value ? <button  class="btn-alt p-xs rnd-s" title="modifie les titres liés" onClick={() => mode.value = 'edit'}>
      ...
      {item.aval.length ? <div>
        <h5>Titre{ item.aval.length > 1 ? 's' : '' } issu de ce titre</h5>
        <div class="flex flex-center">
          <ul class="list-inline" style="margin-bottom: 0">
            { item.aval.map(titreTo => <li key={titreTo.id} class="mr-xs">
