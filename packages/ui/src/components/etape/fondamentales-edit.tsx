...
{canEditDates(props.titreTypeId, props.demarcheTypeId, props.etape.type.id, props.user) ? (
  ...
  <div class="tablet-blobs">
        ...
        <HeritageEdit prop={props.etape.heritageProps.dateDebut} class="tablet-blob-2-3" propId="dateDebut" write={() => (<><InputDate initialValue={props.etape.dateDebut} dateChanged={dateDebutChanged} class="mb-s" />
            ...
            </>)} read={( heritagePropEtape ) => (<div class="border p-s mb-s bold">
...
{canEditTitulaires(props.titreTypeId, props.user) ? (
  ...
  <HeritageEdit prop={props.etape.heritageProps.titulaires} propId="titulaires" write={() => (<><AutocompleteEntreprise
      ...
      />
      <div class="h6 mt-s">
        {titulairesLength.value ? (       <label>
          <input v-model={props.etape.incertitudes.titulaires} type="checkbox" class="mr-xs" />
          Incertain
        </label>) : null}
 
      </div></>)} read={( heritagePropEtape ) => (<ul class="list-prefix">
        {heritagePropEtape?.titulaires.map(t => (<li key={t.id}>
        { getEntrepriseNom(t) }
...
<HeritageEdit prop={props.etape.heritageProps.amodiataires} propId="amodiataires" write={() => (<>
    ...
    />
    <div class="h6 mt-s">
      {amodiatairesLength.value ? (<label>
        <input v-model={props.etape.incertitudes.amodiataires} type="checkbox" class="mr-xs" />
        Incertain
      </label>) : null}

    </div>
    </>)} read={( heritagePropEtape ) => ( <ul class="list-prefix">
      {heritagePropEtape?.amodiataires.map(t => (<li key={t.id}>
      { getEntrepriseNom(t) }
      {t.operateur ? (<Tag mini={true} color="bg-info" class="ml-xs" text="Opérateur" />) : null}
