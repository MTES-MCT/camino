...
const substanceMoveUp = (index: number): void => {
  ...
  return () => (<div>
    ...
    <HeritageEdit prop={props.heritageProps.substances} propId="substances" write={() => (
<>
{props.substances.map((_substance, n) => (<div key={n}>
          <div class="flex mb-s">
            <select v-model={props.substances[n]} class="p-s mr-s">
              {substancesByDomaine.value.map((s) => (<option key={s.id} value={s.id} disabled={props.substances.some(substanceId => substanceId === s.id)}>
                { s.nom }
              </option>))}
            </select>
...
{substancesLength.value && n > 0 && props.substances[n] ? (<button
        ...
        {props.substances?.every(substanceId => !!substanceId) ? <button class="btn small rnd-xs py-s px-m full-x flex mb-s" onClick={substanceAdd}>
          <span class="mt-xxs">Ajouter une substance</span>
          <Icon name="plus" size="M" class="flex-right" />
        </button> : null}

        {substancesLength.value ? <div class="h6">
          <label>
