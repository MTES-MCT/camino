...
const HeritageEditGeneric =  <P extends EtapeHeritageProps, T extends EtapeHeritage>() => caminoDefineComponent<Props<P, T>>(['prop', 'propId', 'write', 'display', 'read'], (props) => {
  ...
  return () => (<div class="mb-s">
    {!props.prop.actif && props.write ? props.write() : (
      ...
      <div class="mb-s">
        {incertain.value ? (<Tag mini={true} color="bg-info" text="Incertain" />) : null}
      </div>
      <p class="h6 italic mb-s">
        Hérité de :
        <span class="cap-first">{ props.prop.etape?.type.nom }</span> ({ dateFormat(props.prop.etape?.date) })
      </p>
  </div>
    )}
  ...
  {props.prop.etape ? (  <button class="btn full-x rnd-xs py-s px-m small mb-s" onClick={() => props.prop.actif = !props.prop.actif}>
    { buttonText.value }
  </button>) : null}
