...
export const up = async (knex: Knex) => {
  ...
  await knex.raw(
    'CREATE TABLE etapes_documents (id character varying(255) NOT NULL, etape_document_type_id character varying(3) NOT NULL, date character varying(10) NOT NULL, etape_id character varying(255) NOT NULL, description character varying(1024), public_lecture boolean NOT NULL, entreprises_lecture boolean NOT NULL, largeobject_id oid)'
  )
  ...
  await knex.raw(
    'insert into etapes_documents (id, etape_document_type_id, date, etape_id, description, public_lecture, entreprises_lecture) (SELECT id, type_id as etape_document_type_id, date, titre_etape_id, description, public_lecture, entreprises_lecture from documents where titre_etape_id is not null and fichier is not null and fichier_type_id is not null)'
  )
