  ...
  for (const etape of etapes.rows) {
    if (!etapesToNotMigrate.includes(etape.id)) {
      ...
      if (isNotNullNorUndefinedNorEmpty(points.rows)) {
        ...
        await knex.raw(`update titres_etapes set geojson4326_perimetre = ST_SetSRID(ST_GeomFromGeoJSON('${JSON.stringify(geojsonFeatureMultiPolygon(points.rows).geometry)}'), 4326), geojson4326_points = ?::jsonb where id = ?`, [JSON.stringify(geojsonPoints), etape.id])
