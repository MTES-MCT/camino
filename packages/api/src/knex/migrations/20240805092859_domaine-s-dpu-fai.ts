...
export const up = async (knex: Knex): Promise<void> => {
  for (const demarcheTypeId of demarcheTypeIdsCxPr_G) {
    await knex.raw(
      `UPDATE titres_etapes te SET statut_id = 'fai' WHERE (select td.type_id  from titres_demarches td where td.id = te.titre_demarche_id) = '${demarcheTypeId}' and type_id = 'dpu' AND slug like 's-cx-%'`
    )
    await knex.raw(
      `UPDATE titres_etapes te SET statut_id = 'fai' WHERE (select td.type_id  from titres_demarches td where td.id = te.titre_demarche_id) = '${demarcheTypeId}' and type_id = 'dpu' AND slug like 's-pr-%'`
    )
  }
}
