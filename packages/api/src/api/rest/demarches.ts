...
export const getDemarcheByIdOrSlug = (pool: Pool) => async (req: CaminoRequest, res: CustomResponse<GetDemarcheByIdOrSlugValidator>) => {
  try {
    ...
    if (!isSuper(user)) {
      ...
      try {
        const result: GetDemarcheByIdOrSlugValidator = await getDemarcheByIdOrSlugDb(pool, demarcheIdOrSlugParsed.data)
        res.json(getDemarcheByIdOrSlugValidator.parse(result))
      } catch (e) {
