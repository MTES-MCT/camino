...
describe('titreDemandeCreer', () => {
  ...
  test('peut créer un titre en tant que entreprise', async () => {
    const tested = await restNewPostCall(dbPool, '/rest/titres', {}, { role: 'entreprise', entreprises: [{ id: entrepriseId }] }, body)
  ...
  test('ne peut pas lier un titre inexistant', async () => {
    const tested = await restNewPostCall(dbPool, '/rest/titres', {}, { role: 'super' }, { ...body,titreTypeId: 'axm', titreFromIds: [titreIdValidator.parse('unknownTitreId'), titreIdValidator.parse('anotherOne')] })
