...
type MyTitreRef = {type: NonNullable<ITitreReference['type']>} & Omit<ITitreReference, 'type'>
...
export const titresONF =  async (
  ...
  if( !user?.administrations?.find(({id}) => id === ADMINISTRATION_IDS['OFFICE NATIONAL DES FORÊTS'])  ){
    ...
    const titres = await titresGet(
      ...
      { fields: { statut: {id: {}}, references: {type: {id: {}}}, titulaires: {id: {}}, demarches: { etapes: {id: {}}} }},
      user
    )
      ...
      const titresFormated: CommonTitreONF[] = titres
        .map<CommonTitreONF | null>((titre: ITitre) => {
          ...
          const references = titre.references.filter((reference: ITitreReference): reference is MyTitreRef => !!reference.type && !!reference.type.nom && !!reference.nom)
        ...
        .filter((titre: CommonTitreONF | null): titre is CommonTitreONF => titre !== null)
