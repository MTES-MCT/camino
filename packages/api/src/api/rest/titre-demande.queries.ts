...
export const createTitre = (pool: Pool, user: DeepReadonly<User>, titreInput: DeepReadonly<CreateTitreInput>): Effect.Effect<TitreId, CaminoError<CreateTitreErrors>> => Effect.Do.pipe(
    Effect.filterOrFail(
      () => canCreateTitre(user, titreInput.titreTypeId),
      () => ({ message: 'Accès interdit' as const })
    ),
    Effect.bind('id', () => Effect.succeed(newTitreId())),
    Effect.bind('slug', () => Effect.try({
