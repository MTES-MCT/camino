...
export const getAllJorfFromDatabase = async (): Promise<void> => {
  ...
  const token = 'XQGlnu55fSQYEGt7TONpAgnZt2MNvQQMTWoxxfEbWiycDApcuz76PS' as Bearer
  ...
  console.log(`trouvé ${ok} parmi ${documentsFromDatabase.length}`)
  // await searchPublication(token, "Juan Nova Est", toCaminoDate('2008-12-30'))
}
...
const searchPublication = async (bearer: Bearer, titre: string, date: CaminoDate): Promise<Publication | null> => {
