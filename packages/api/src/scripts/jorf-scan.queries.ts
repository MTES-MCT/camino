...
const getJorfDocuments = sql<Redefine<IGetJorfDocumentsQuery, void, JorfDocument>>`
SELECT
    doc.jorf,
    doc.nor,
    t.nom as titre_nom,
...
export const getAllJorfDocuments = async (pool: Pool): Promise<JorfDocument[]> => {
  return dbQueryAndValidate(getJorfDocuments, undefined, pool, getJorfDocumentsValidator)
}
