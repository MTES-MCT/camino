...
describe('trie les étapes', () => {
  ...
  test('des étapes organisées par date décroissante sont triées par date croissante', () => {
    expect(
      titreEtapesSortAscByDate(titreEtapesSortedDesc, newDemarcheId(), 'oct', 'prh')
    ).toMatchObject(titreEtapesSortedAsc)
  })

  test('des étapes organisées par date croissante restent triées par date croissante', () => {
    expect(
      titreEtapesSortAscByDate(titreEtapesSortedAsc, newDemarcheId(), 'oct', 'prh')
    ).toMatchObject(titreEtapesSortedAsc)
  })
  ...
  test('des étapes avec les mêmes dates organisées par ordre décroissant sont triées par ordre croissant', () => {
    const titreEtapesMemesDatesOrdreDesc: Pick<ITitreEtape, "ordre" | "typeId" | "statutId" | "date" | "contenu" | "titreDemarcheId">[] = [
      { typeId: 'dex', ordre: 2, date: toCaminoDate('1988-03-06'), titreDemarcheId: newDemarcheId(), statutId: 'fai' },
      { typeId: 'dpu', ordre: 1, date: toCaminoDate('1988-03-06'), titreDemarcheId: newDemarcheId(), statutId: 'fav' }
    ]
  ...
  test('des étapes avec les mêmes dates sont triées par ordre de type croissant', () => {
    ...
    const titreEtapesMemesDatesOrdreEtapesTypesDesc: Pick<ITitreEtape, "ordre" | "typeId" | "statutId" | "date" | "contenu" | "titreDemarcheId">[] = [
      { typeId: ETAPES_TYPES.initiationDeLaDemarcheDeRetrait, ordre: 2, date: toCaminoDate('1988-03-06'), titreDemarcheId, statutId: 'fav' },
      { typeId: ETAPES_TYPES.classementSansSuite, ordre: 2, date: toCaminoDate('1988-03-06'), titreDemarcheId, statutId: 'fav' },
      { typeId: ETAPES_TYPES.decisionAdministrative, ordre: 2, date: toCaminoDate('1988-03-06'), titreDemarcheId, statutId: 'fav' },
    ]
    expect(
      titreEtapesSortAscByDate(
      ...
      { typeId: ETAPES_TYPES.initiationDeLaDemarcheDeRetrait, ordre: 2, date: '1988-03-06', statutId: 'fav', titreDemarcheId },
      { typeId: ETAPES_TYPES.decisionAdministrative, ordre: 2, date: '1988-03-06', statutId: 'fav', titreDemarcheId },
      { typeId: ETAPES_TYPES.classementSansSuite, ordre: 2, date: '1988-03-06', statutId: 'fav', titreDemarcheId }
    ])
  })
  ...
  test('tri selon l’arbre si les étapes ont la même date', () => {
    const etapes: Pick<ITitreEtape, "ordre" | "typeId" | "statutId" | "date" | "contenu" | "titreDemarcheId">[] = [
      { typeId: 'pfd', date: toCaminoDate('2020-01-01'), statutId: 'fai', titreDemarcheId: newDemarcheId() },
      { typeId: 'mfr', date: toCaminoDate('2020-01-01'), statutId: 'fai', titreDemarcheId: newDemarcheId() },
      { typeId: 'mdp', date: toCaminoDate('2020-01-01'), statutId: 'fai', titreDemarcheId: newDemarcheId() }
    ]
  ...
  test("retourne une erreur si le type d'étape est absent dans la définition", () => {
    const etapes: Pick<ITitreEtape, "ordre" | "typeId" | "statutId" | "date" | "contenu" | "titreDemarcheId">[]  = [
      { typeId: 'mcr', date: toCaminoDate('2020-01-01'), statutId: 'fai', titreDemarcheId: newDemarcheId() },
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      { typeId: 'bof', date: toCaminoDate('2020-01-01'), statutId: 'fai', titreDemarcheId: newDemarcheId() },
      { typeId: 'vfd', date: toCaminoDate('2020-01-01'), statutId: 'fai', titreDemarcheId: newDemarcheId() }
    ]
