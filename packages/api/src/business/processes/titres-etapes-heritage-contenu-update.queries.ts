...
export const getEtapesByDemarcheValidator = z.object({ contenu: z.any(),
  ...
  SELECT titre.id as titre_id, titre.type_id as titre_type_id, etape.id, etape.ordre, etape.type_id, etape.statut_id, etape.date, etape.contenu, etape.heritage_contenu, demarche.id as demarche_id, demarche.type_id as demarche_type_id, demarche.statut_id as demarche_statut_id, etape.communes as communes
  from titres_etapes etape
      ...
      and ($demarcheId::TEXT IS NULL or demarche.id = $demarcheId)
      and ($titreId::TEXT IS NULL or titre.id = $titreId)
