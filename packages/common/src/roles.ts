...
type UserEntreprise = { role: 'bureau dՎtudes' } & EntrepriseUserNotNull
...
export type EntrepriseOrBureauDetudeRole = Extends<
  Role,
...
export type UserNotNull =
  | UserSuper
  | UserDefaut
  | AdminUserNotNull
  | EntrepriseUserNotNull

export const isAdministrationRole = (role: Role): role is AdministrationRole =>
  ['admin', 'editeur', 'lecteur'].includes(role)
export const isAdministrationAdminRole = (role: Role): role is 'admin' =>
  role === 'admin'
export const isSuperRole = (role: Role): role is 'super' => role === 'super'
export const isEntrepriseOrBureauDetudeRole = (
  role: Role
): role is EntrepriseOrBureauDetudeRole =>
  ['entreprise', 'bureau dՎtudes'].includes(role)
