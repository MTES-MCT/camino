...
describe('role', () => {
  ...
  test('isSuper', () => {
    ...
    expect(isSuper({ ...baseRole, role: 'entreprise', entreprises: [] })).toBe(
      false
    )
  ...
  test('isAdministration', () => {
    ...
    expect(
      isAdministration({ ...baseRole, role: 'admin', administrationId })
    ).toBe(true)
    expect(
      isAdministration({ ...baseRole, role: 'editeur', administrationId })
    ).toBe(true)
    expect(
      isAdministration({ ...baseRole, role: 'lecteur', administrationId })
    ).toBe(true)
  })
  test('isAdministrationAdmin', () => {
    expect(
      isAdministrationAdmin({ ...baseRole, role: 'admin', administrationId })
    ).toBe(true)
    expect(
      isAdministrationAdmin({ ...baseRole, role: 'editeur', administrationId })
    ).toBe(false)
  })
